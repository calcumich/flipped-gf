...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.distributed:
            ...
            data_loader_val.sampler.set_epoch(epoch)   # don't think this is eating up memory
        print(f"Before train_one_epoch, epoch {epoch}")
        misc.printMemory() 
        ...
        print(f"After train_one_epoch, before val, epoch {epoch}")
        misc.printMemory() 
        val_stats = val_one_epoch(model_without_ddp, data_loader_val, optimizer, epoch, args=args)
        print(f"After val_one_epoch, epoch{epoch}")
        misc.printMemory() 
        if args.output_dir and best_acc < val_stats['acc']:   # how often are we entering this?  is misc.save_model eating memory?
            ...
            print(f"After saving best model, epoch {epoch}")
            misc.printMemory()
